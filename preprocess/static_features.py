...
# add pos_tag-word_tokenize with underthesea or bert
...
class NERdataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if sample.label_ids is not None:
            ...
            label_id_tensors = None
            label_mask_tensors=None
        feat_tensors = {}
...
class FeatureExtractor:
    def __init__(self, dict_dir: str, feature_types: list = None):
        if feature_types is None:
            feature_types = [ "cf", "sc", "fw", "qb", "num", "loc", "org", "per", "ppos"]
    ...
    def wseg_and_add_pos_tag_feature(sentence: str or list,
                                     pos_tags: list = None,
                                     ner_labels: list = None) :
        # print("call wseg", sentence)
    ...
    def extract_feature(self, sentence: str or list, pos_tags: list = None, ner_labels: list = None, format=None,is_segmentation=False):
        features = []
        result = []
        if "pos" in self.feature_types and is_segmentation is False:
            ...
            # print("here1")
        elif is_segmentation is False:
            # print("here2")
            sentence, ner_labels = self.word_segment(sentence, ner_labels)
        else:
            # print("here3")
            sentence, ner_labels = sentence, ner_labels
        ...
        if format == "text":
            # print(len(ner_labels),len(sentence))
