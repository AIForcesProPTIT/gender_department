...
class NERProcessor:
    ...
    def convert_examples_to_features(self, examples, max_seq_length, feature=None):
        ...
        for (ex_index, example) in tqdm(enumerate(examples), total=len(examples)):
            ...
            for i, (word, label) in enumerate(zip(ex_words, ex_labels)):
                ...
                for m in range(len(token)):
                    ...
                    if m == 0:
                        ...
                        next_label = label if 'O' in label else 'I-' +label.split("-")[-1]
            ...
            # assert sum(token_masks) == sum(label_masks), (token_masks, label_masks)
            for k in feats.keys():
            ...
            features.append(
                Example(eid=example[0],
                        ...
                        feats=feats))
        return features
    def convert_sentences_to_features(self, examples, max_seq_length, feature=None):
        features = []
        for (ex_index, example) in tqdm(enumerate(examples), total=len(examples)):
            ex_id, ex_words, ex_feats = example
            # Init Example features
            tokens = []
#             labels = []
            feats = {}
